# -*- coding: utf-8 -*-
# CHIMBITA-BOT — Railway Cron (Fixed, English-only, no railway.json)

import ccxt, pandas as pd, numpy as np, requests, time, os
from datetime import datetime, timezone

TELEGRAM_TOKEN = os.getenv("TELEGRAM_TOKEN", "8364292253:AAE7zlGjDqGV63_0DuILVVJZCFb91igxA8Y")
CHAT_ID = os.getenv("CHAT_ID", "1982879600")
...
def build_exchange_with(endpoint: str):
...
def get_exchange_resilient():
    last_exc = None
    for ep in BINANCE_PUBLIC_ENDPOINTS:
        try:
            ex = build_exchange_with(ep)
...
def fetch_ohlcv_safe(ex, symbol, timeframe, limit):
    for _ in range(3):
        try:
            ...
            if DEBUG: print(f"[ERR] fetch_ohlcv {symbol} {timeframe}: {e}")
            break
    return None
